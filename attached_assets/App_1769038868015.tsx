...
const App: React.FC = () => {
  ...
  // Helper to safely open one menu while closing others to prevent UI overlap
  const toggleMenu = useCallback((menu: 'style' | 'settings' | 'log') => {
    if (menu === 'style') {
  ...
  }, [complexityLevel, visualStyle, language, isAdvancedMode, variationCount, imageQuality, aspectRatio, currentSuggestions, loadingFacts]);

  // --- Storage Helpers ---
  
  const saveHistoryToStorage = useCallback((history: GeneratedImage[]) => {
    try {
      if (history.length === 0) {
      ...
      localStorage.setItem('infoGeniusHistory', serialized);
    } catch (e: any) {
      if (e.name === 'QuotaExceededError' || e.name === 'NS_ERROR_DOM_QUOTA_REACHED' || e.code === 22) {
        if (history.length > 1) {
          const reducedHistory = history.slice(0, history.length - 1);
          saveHistoryToStorage(reducedHistory);
        } else if (history.length === 1) {
          localStorage.removeItem('infoGeniusHistory');
        }
      } else {
        logError(e, 'HistorySave_Unexpected');
      }
    }
  }, []);
  ...
  }, [imageHistory, saveHistoryToStorage]);
  ...
  return (
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-200 font-sans selection:bg-cyan-500 selection:text-white pb-20 relative overflow-x-hidden animate-in fade-in duration-1000 transition-colors">
      ...
      <header className="border-b border-slate-200 dark:border-white/10 sticky top-0 z-[60] backdrop-blur-md bg-white/70 dark:bg-slate-950/60 transition-colors">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 md:h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
              ...
              <div className="relative" ref={settingsMenuRef}>
                <button onClick={() => toggleMenu('settings')} className={`p-2 rounded-full transition-colors border shadow-sm group ${isSettingsMenuOpen ? 'bg-cyan-50 dark:bg-slate-800 border-cyan-200 dark:border-white/20 text-cyan-600 dark:text-white' : 'bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-400 hover:text-cyan-600 dark:hover:text-cyan-300'}`} title="Settings"><Settings className="w-5 h-5 transition-transform duration-300 group-hover:rotate-90" /></button>
                {isSettingsMenuOpen && (
                    <div className="absolute top-full right-0 mt-4 w-96 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-slate-200 dark:border-white/10 p-6 z-[70] animate-in fade-in zoom-in-95 duration-200 origin-top-right ring-1 ring-black/5 dark:ring-white/5 max-h-[85vh] overflow-y-auto custom-scrollbar">
            ...
            <div className="mt-6 flex flex-col items-center">
              <div className="flex items-center gap-4 mb-4">
                ...
                {imageHistory.length > 0 && (
                  <button type="button" onClick={() => toggleMenu('log')} className={`px-4 py-1.5 rounded-full text-[11px] font-bold flex items-center gap-2 shadow-lg hover:scale-105 transition-all ${isLogExpanded ? 'bg-cyan-600 dark:bg-cyan-500 text-white' : 'bg-slate-900 dark:bg-white text-white dark:text-slate-900'}`}>
              ...
              {isLogExpanded && imageHistory.length > 0 && (
                <div className="w-full max-w-2xl bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl rounded-2xl border border-slate-200 dark:border-white/10 p-4 shadow-2xl animate-in slide-in-from-top-4 duration-300 z-30 ring-1 ring-black/5">
                  <div className="flex items-center justify-between mb-4 px-2 border-b border-slate-100 dark:border-white/5 pb-3">
                  ...
                  <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                    {imageHistory.map((item) => (
                      <button key={item.id} onClick={() => { restoreImage(item); setIsLogExpanded(false); }} className="w-full text-left p-3 rounded-xl bg-slate-50/80 dark:bg-slate-800/60 border border-transparent hover:border-cyan-500/30 hover:bg-white dark:hover:bg-slate-800 transition-all group flex items-center justify-between">
